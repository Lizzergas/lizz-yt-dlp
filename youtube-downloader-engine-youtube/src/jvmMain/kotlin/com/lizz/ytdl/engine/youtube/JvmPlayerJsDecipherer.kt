...
internal class JvmPlayerJsDecipherer {
    fun resolveCipheredFormats(
        formats: List<NativeAudioFormat>,
        playerJs: String,
    ): List<NativeAudioFormat> {
        val solver = SignatureSolver.tryCreate(playerJs) ?: return formats
    ...
    private fun resolveCipheredFormat(
        ...
        val finalUrl = appendQuery(baseUrl, sp, signature)
        return format.copy(url = finalUrl)
    }
    ...
    private fun parseQueryString(value: String): Map<String, String> {
        return value.split('&').mapNotNull { token ->
            ...
            val key = URLDecoder.decode(token.substring(0, eq), StandardCharsets.UTF_8)
            val decodedValue = URLDecoder.decode(token.substring(eq + 1), StandardCharsets.UTF_8)
            key to decodedValue
        }.toMap()
    }
    ...
    companion object {
        ...
        private fun extractFunctionBody(playerJs: String): String? {
            ...
            return patterns.firstNotNullOfOrNull { regex ->
                regex.find(playerJs)?.groupValues?.getOrNull(3)
            }
        }
